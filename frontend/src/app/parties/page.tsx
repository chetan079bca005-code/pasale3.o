...
export default function PartiesPage() {
    ...
    return transactions
      .filter(
        (tx) =>
          ...
          (tx.partyName && tx.partyName.toLowerCase() === selectedParty.name.toLowerCase())
      )
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, 5);
  ...
  const lastTransactionDate = partyTransactions.length > 0
    ? formatDate(partyTransactions[0].date, language)
    : null;

  const handleDeleteParty = (partyId: string) => {
    if (confirm(t('parties.deleteConfirm'))) {
      // Note: Delete functionality would need to be implemented in the data store
      alert(t('parties.deleteNotImplemented'));
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 overflow-x-hidden">
      <div className="w-full max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 py-4 sm:py-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="group flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl bg-linear-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 border border-blue-100 dark:border-blue-800/30 hover:shadow-lg transition-all duration-300 cursor-default flex-1">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                ...
                <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300">
                  {n(parties.length)} {t('parties.contacts') || 'Contacts'}
                </span>
              </h1>
          ...
          <Button
            ...
            className="bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 transition-all text-xs sm:text-sm shrink-0"
          >
        ...
        {selectedParty && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
            <div className="bg-white dark:bg-gray-800 w-full max-w-3xl max-h-[90vh] rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden overflow-y-auto animate-in fade-in zoom-in duration-200">
              {/* Header */}
              <div className={`p-4 sm:p-6 text-white ${selectedParty.type === 'customer'
                  ? 'bg-linear-to-r from-blue-500 to-blue-600'
                  : 'bg-linear-to-r from-purple-500 to-purple-600'
                }`}>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center text-lg sm:text-2xl font-bold">
                      {selectedParty.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h2 className="text-lg sm:text-2xl font-bold">{selectedParty.name}</h2>
                      <p className="text-white/80 capitalize mt-0.5 sm:mt-1 text-sm sm:text-base">
                        {selectedParty.type === 'customer' ? t('parties.customer') : t('parties.supplier')}
                      </p>
                    </div>
                  </div>
                  <button
              ...
              <div className="p-4 sm:p-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <h3 className="text-[10px] sm:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      {t('common.contactInfo')}
                    </h3>
                    <div className="space-y-1.5 sm:space-y-2">
                      <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                        <FiPhone className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
                        <span className="text-gray-900 dark:text-gray-100 truncate">
                          {selectedParty.phone || '—'}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                        <FiMail className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
                        <span className="text-gray-900 dark:text-gray-100 truncate">
                          {selectedParty.email || '—'}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
                        <FiMapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 shrink-0" />
                        <span className="text-gray-900 dark:text-gray-100 truncate">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    <h3 className="text-[10px] sm:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      {t('parties.transactionInfo')}
                    </h3>
                    <div className="space-y-1.5 sm:space-y-2">
                      <div>
                        <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">{t('parties.totalTransactions')}</p>
                        <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm sm:text-base">
                          {n(transactions.filter(t => t.partyId === selectedParty.id).length)}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">{t('parties.lastTransaction')}</p>
                        <p className="font-semibold text-gray-900 dark:text-gray-100 text-sm sm:text-base">
                          {lastTransactionDate || t('common.never')}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Balance Card */}
                  <div className={`p-3 sm:p-4 rounded-xl sm:rounded-2xl sm:col-span-2 lg:col-span-1 ${selectedParty.balance >= 0
                      ? 'bg-emerald-50 dark:bg-emerald-900/20'
                      : 'bg-red-50 dark:bg-red-900/20'
                    }`}>
                    <p className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-0.5 sm:mb-1">
                      {t('parties.currentBalance')}
                    </p>
                    <p className={`text-xl sm:text-2xl lg:text-3xl font-bold ${selectedParty.balance >= 0
                        ? 'text-emerald-600 dark:text-emerald-400'
                        : 'text-red-600 dark:text-red-400'
                      }`}>
                      {c(Math.abs(selectedParty.balance))}
                    </p>
                    <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mt-0.5 sm:mt-1">
                      {selectedParty.balance >= 0 ? t('parties.toReceive') : t('parties.toPay')}
                    </p>
                  </div>
                </div>
                ...
                {partyTransactions.length > 0 && (
                  <div className="mb-4 sm:mb-6">
                    <h3 className="text-[10px] sm:text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 sm:mb-3">
                      {t('parties.recentTransactions')}
                    </h3>
                    <div className="space-y-1.5 sm:space-y-2 max-h-48 overflow-y-auto scrollbar-none">
                      {partyTransactions.map((tx) => (
                        <div
                          key={tx.id}
                          className="flex items-center justify-between p-2 sm:p-3 bg-gray-50 dark:bg-gray-900/50 rounded-lg sm:rounded-xl"
                        >
                          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 ${tx.type === 'selling'
                                ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600'
                                : 'bg-blue-100 dark:bg-blue-900/30 text-blue-600'
                              }`}>
                              {tx.type === 'selling' ? <FiArrowUpRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <FiArrowDownLeft className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                            </div>
                            <div className="min-w-0">
                              <p className="font-medium text-gray-900 dark:text-gray-100 text-xs sm:text-sm truncate">
                                {tx.description}
                              </p>
                              <p className="text-[10px] sm:text-xs text-gray-500">{d(tx.date)}</p>
                            </div>
                          </div>
                          <p className={`font-bold text-xs sm:text-sm shrink-0 ml-2 ${tx.type === 'selling' ? 'text-emerald-600' : 'text-gray-900 dark:text-gray-100'
                            }`}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                  <Button
                    size="sm"
                    onClick={() => {
                      navigate(`/billing?partyId=${selectedParty.id}`);
                      setSelectedPartyId(null);
                    }}
                    className="bg-linear-to-r from-blue-600 to-blue-700 text-white text-xs sm:text-sm"
                  >
                    <FiPrinter className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
                    <span className="hidden sm:inline">{t('billing.createBill')}</span>
                    <span className="sm:hidden">Bill</span>
                  </Button>
                  <Button
                    ...
                    onClick={() => {
                      const amount = prompt(t('billing.enterPaymentAmount'));
                      if (amount) {
                        alert(`${t('billing.paymentRecorded')} ${c(parseFloat(amount))}`);
                        setSelectedPartyId(null);
                      }
                    }}
                  >
                    <FiCreditCard className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
                    <span className="hidden sm:inline">{t('billing.recordPayment')}</span>
                  ...
                  <Button
                    ...
                    className="text-xs sm:text-sm"
                    onClick={() => {
                      navigate(`/ledger/${selectedParty.id}`);
                      setSelectedPartyId(null);
                    }}
                  >
                    <FiFileText className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
                    <span className="hidden sm:inline">{t('parties.viewLedger')}</span>
                    <span className="sm:hidden">Ledger</span>
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    className="text-red-600 border-red-200 hover:bg-red-50 dark:border-red-800 dark:hover:bg-red-900/20 text-xs sm:text-sm"
                    onClick={() => handleDeleteParty(selectedParty.id)}
                  >
                    <FiTrash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
