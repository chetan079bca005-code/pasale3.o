...
import random
from .models import Customer, Party, Product, Supplier, UserProfile, Expense, Billing, BillingItem, UserSettings
from .serializers import ProductSerializer, PartySerializer, CustomerSerializer, SupplierSerializer, ExpenseSerializer, BillingSerializer, BillingItemSerializer, UserSettingsSerializer, UserProfileSerializer
...
# Helper function to send OTP email (fallback if Celery fails)
def send_otp_email_sync(email, otp):
    """
    Synchronous email sending fallback.
    Used if Celery is not running or task fails.
    """
    try:
...
class SignupView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not all([username, email, password, phone_no, business_name]):
        ...
        try:
            with transaction.atomic():
                ...
                # Try to send OTP email via Celery first (async)
                # If Celery is not running, fall back to synchronous email
                try:
                    send_otp_email.delay(email, otp)
                    logger.info(f"OTP email task queued for {email}")
                except Exception as celery_error:
                    logger.warning(f"Celery failed, using sync email: {celery_error}")
            ...
            return Response({'error': f'Signup failed: {str(e)}'}, 
                          status=status.HTTP_400_BAD_REQUEST)
...
class VerifySignupOtpView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if str(user_profile.otp) == str(otp_provided):
            ...
            # Generate tokens after successful signup verification
...
class ApiPartyView(APIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        data = request.data
        party.photo = data.get('photo', party.photo)
